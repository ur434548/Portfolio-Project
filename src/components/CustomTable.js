...
const CustomTable = ({
  ...
  return (
    <div className="bg-white shadow-md  rounded-2xl p-4 mx-3 h-[50.6rem]">
      ...
      <div className="max-h-[707px] custom-scrollbar overflow-y-auto">
        <table className={`container bg-white ${className}`}>
          ...
          <tbody className="bg-white divide-y divide-slate-200 ">
            {data.map((item) => (
              <tr key={item.id}>
                <td className="py-2 px-3 text-xs h-auto sm:text-sm md:text-base font-medium text-gray-500">
                  {item.firstColVal}
                </td>
                <td className="py-2 px-3 text-xs h-auto sm:text-sm md:text-base text-gray-500">
                  {item.secondColVal}
                </td>
                <td className="py-2 px-3 text-xs h-auto sm:text-sm md:text-base text-gray-500">
                  {item.thirdColVal}
                ...
                <td className="py-2 px-5 text-xs sm:text-sm items-center content-center cursor-pointer md:text-base font-medium text-indigo-600 flex  gap-2">
                  {icons.map((icon, index) => (
                    <span key={index} onClick={icon.onClick}>
                      {icon.icon}
                    </span>
                  ))}
                </td>
